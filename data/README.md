...
- `eksik_alanlar`: Evrakta gerçekten eksik olan alanların listesi; alan adları eksik bilgi tespit modülüyle (`src/agents/missing_info_agent.py` içindeki `ZORUNLU_ALANLAR`) **birebir** uyumlu olmalıdır — bu modül tek doğruluk kaynağıdır. **Dikkat:** tutanak imzaları için `imza` değil `imzalar` kullanılır (tür-özel anahtar). Sık örnekler: `tarih`, `sayi`, `konu`, `muhatap`, `ilgi`, `imza`, `imzalar`, `ad_soyad`, `tc_kimlik`, `adres`, `kurum_bilgisi`, `dagitim`, `katilimcilar`, `hazirlayan`. Tam evraklarda boş listedir.
