...
Etiketli sentetik değerlendirme setlerinde sistem; **yeni tutulmuş sette (v2)** evrak sınıflandırmada **%100 doğruluk (16/16)**, birim yönlendirmede **%93,8 doğruluk (15/16)**, eksik bilgi tespitinde **1,000 micro-F1**, mevzuat önerisinde **0,750 isabet@3** başarımına ve evrak başına **~0,012 saniyelik** medyan işleme süresine (gerçek zamana yakın) ulaşmıştır; geliştirme setinde sınıflandırma ve eksik bilgi 1,00, yönlendirme 0,962, mevzuat isabet@3 0,962; ilk tutulmuş sette sınıflandırma, yönlendirme ve eksik bilgi 1,00, mevzuat isabet@3 0,875'tir (bkz. Bölüm 5 ve metodolojik notlar).
...
**Bağımsız taslak kalite hakemi:** Format öz-denetiminin üzerine, üretici ajandan bağımsız bir hakem her taslağı **0-100 ölçeğinde** puanlar (`src/utils/taslak_hakemi.py`). LLM erişilebilirse dört boyutlu rubrik (üslup/yapı/kapanış/açıklık) LLM-as-judge ile puanlatılır; erişilemezse kural tabanlı eşdeğer (biçim %40 + üslup %30 + mevzuat temellilik %30) aynı ölçeğe normalize edilir — iki yolun çıktısı karşılaştırılabilirdir. **Mevzuat temellilik** bileşeni her iki yolda da deterministiktir (RAGAS'ın groundedness fikrinin saf Python karşılığı): taslakta atıf yapılan mevzuat, önerici ajanın mutlak benzerlik skorlarıyla desteklenmeli ve öneri listesinde bulunmalıdır; **listede olmayan atıf halüsinasyon işareti sayılıp ağır cezalandırılır** (temellilik yargısının LLM'e bırakılmaması bilinçlidir — halüsinasyon halüsinasyonla denetlenmez). Çevrimdışı kural modunda ölçülen ortalama kalite puanları: geliştirme 93,6 / tutulmuş 95,8 / tutulmuş v2 94,6 (bkz. Bölüm 5).
...
2. **KVKK Anonimleştirme (Paylaşım Nüshası):** 6698 sayılı Kanun bağlamında; T.C. kimlik (checksum doğrulamalı), telefon, e-posta, IBAN, kişi adı, adres, araç plakası (il kodu doğrulamalı), doğum tarihi ("Doğum Tarihi" etiketiyle bağlam-sınırlı) ve personel sicil numarası bilgilerini format-koruyan biçimde maskeleyerek evrakın birim/kurum arası paylaşıma uygun nüshasını üretir; kurum adları, unvanlar ve tüzel kişi sicilleri (Ticaret/Vergi/Tapu Sicil No) tüzel kişi verisi olduğundan maskelenmez.
...
- **Geliştirme seti:** 52 etiketli sentetik evrak (8 tür; 9 birimin her biri en az 2 evrakta hedef; her türden en az 1 kasıtlı eksik alanlı dosya). 12.07.2026'da CİMER kaynaklı başvuru, dağıtımlı yazı, `GÜNLÜDÜR`/`ACELE` damgalı süreli yazı ve tür çeşitliliği desenleriyle 35'ten 52'ye genişletilmiştir (üretim + bağımsız çift-etiketleme; ayrıntı `data/README.md`). Takım üretimi, Apache 2.0.
...
- **Mevzuat korpusu:** 15 belge — 3071, 4982, 6698, 5070, 657, 5018, 4734, 2577, 5326, 5393, 3194 sayılı kanunların ve Resmî Yazışmalar ile Devlet Arşiv Hizmetleri yönetmeliklerinin evrak işleme bağlamındaki hükümlerinin özgün cümlelerle yazılmış özetleri + CİMER ve e-Yazışma bilgi notları (kaynak: mevzuat.gov.tr, cimer.gov.tr, DDO). Ayrıca Standart Dosya Planı (2005/7 sayılı Başbakanlık Genelgesi, 25.03.2005) birincil kaynaktan doğrulanıp `docs/standart_dosya_plani_notu.md` referans belgesine işlenmiştir; **bilinçli olarak retrieval korpusuna alınmamıştır** çünkü SDP bir dosya-kodlama standardıdır (içerik-bazlı kanun önerisinden işlevsel farklı) ve etiketli evrakların hiçbiri onu `mevzuat_beklenen` olarak işaret etmediğinden korpusa eklendiğinde ölçülen etki geliştirme isabet@3'ünü 0,962→0,923 düşürmüştür (yer gerçeği karşılığı olmadan retrieval gürültüsü) — bkz. adı geçen belge.
...
| Evrak başına medyan süre | 0,012 sn |

**Hata analizi (karışma desenleri):** (a) *Sınıflandırma* — tek hata `ust_yazi_a1` → cevap_yazisi: bozuk sayı bloklu üst yazının İlgi satırındaki "... sayılı yazınız" ikinci-şahıs iyelikli atfı, cevap yazısının temel ayrım sinyalini tetiklemiştir (bilinen sinyalin sınır durumu; confusion matrix raporun `siniflandirma.confusion_matrix` alanındadır). (b) *Eksik bilgi* — üç yanlış alarm ve bir kaçırma tamamen adversarial tuzakların hedefindedir: `cevap_yazisi_a1`'de İlgi bloğu yokken gövdedeki "İlgi (b)'de kayıtlı yazınız" ifadesi alanın VAR sanılmasına yol açmış (kopuk zincir, FN); `tutanak_a1`'deki tamamen sözel tarih ("Temmuz ayının on ikinci günü") çıkarılamamış (FP); iki rapor dosyasında farklı cümle iskeleti kullanıldığından fiil-kökü tabanlı sonuç/değerlendirme denetimi yanlış alarm üretmiştir (FP×2). (c) *Mevzuat* — `tutanak_a2` KVKK-yoğun içerik taşımasına rağmen "kişisel/kvkk/rıza" tetikleyici kelimeleri geçmediğinden kisisel_veri teması aktifleşmemiş, 6698 önerilememiştir; `rapor_a1`'de 5018 yerine arşiv/RYY önerilmiştir. (d) *Güçlü yanlar* — yönlendirme adversarial sette dahi 16/16'dır; iki çift-doğalı makam oluru da doğru birime yönlendirilmiştir; taslak kalitesi ve hız etkilenmemiştir. Bu bulgular İLK ölçümde (13.07.2026 öncesi kod) düzeltilmeden, olduğu gibi raporlanmıştır. **Sonradan** bu sınırlılıklardan üçü (a: İlgi yapısal denetimi, b: sözel tarih, d: KVKK veri-sinyali) ilkesel olarak giderilmiştir (§5.1.1). v3 bu düzeltmelerin hata analizi kaynağı olduğundan artık *geliştirme-bilgili*dir: güncel kodla yeniden ölçümünde eksik-bilgi F1 0,667→**0,833** ve isabet@3 0,875→**0,9375** yükselir (`eval_report_heldout_v3.json`). Değerlendirme bütünlüğü kuralı gereği (CLAUDE.md, `data/README.md` §2), iyileştirilmiş sistemin DOKUNULMAMIŞ ölçümü **§5.1.1'deki yeni adversarial held-out set v4**'tür; kalan (c) ve (e) sınırlılıkları düzeltilmemiş olup v4'te de gözlenmiştir (Bölüm 6).

#### 5.1.1. İlkesel İyileştirmeler ve Temiz Yeniden Ölçüm (Adversarial Held-out v4)

v3'ün (§5.1) ortaya koyduğu üç sınırlılık, **held-out hatasına dosyaya özel kural yazmadan**, genel/ilkesel kurallarla giderilmiştir (tümü geliştirme setinde ve 19 yeni birim testinde doğrulanmış; dev/tutulmuş/v2 metriklerinde **sıfır regresyon**):

1. **Yapısal İlgi denetimi** — "İlgi" bir alan etiketi olarak yalnızca iki nokta ile tanımlanır (`^\s*İlgi\s*:`); gövdedeki düz cümle atıfları ("İlgi (b)'de kayıtlı yazınız") artık İlgi bloğu sayılmaz, dolayısıyla kopuk İlgi zinciri yapısal olarak tespit edilir.
2. **Sözel tarih çözümü** — rakamsal tarih içermeyen belgelerde `"<Ay> ayının <gün-yazı> günü"` kalıbı (birler/onlar + sıra biçimleri, 1-31) evrak tarihi olarak çözülür.
3. **KVKK veri-sinyali köprüsü** — belge doğrulanmış (checksum'lı) bir T.C. kimlik numarası veya IBAN içeriyorsa, 6698 sayılı KVKK "kişisel/kvkk" sözcükleri geçmese bile öneri listesinin ilk üçüne alınır; enjekte edilen öneri metinsel eşleşme iddia etmez (benzerlik taslak atıf eşiğinin altında raporlanır, `eklenme_nedeni="kvkk_veri_sinyali"` ile şeffaf işaretlenir).

İyileştirilmiş sistem, **tamamen dokunulmamış, bağımsız çift-etiketlemeli yeni bir adversarial set (v4)** ile temiz ölçülmüştür: beşinci kurgu evren (sahil/liman/turizm ekonomili "Deniztepe" ili / "Fenerburnu" ilçesi), 16 evrak; düzeltilen üç desenin taze örnekleri (kopuk İlgi, tamamen sözel tarih, KVKK-yoğun sözcüksüz üçüncü-şahıs verisi) ile yeni tuzaklar (bozuk sayı bloğu, çift-doğalı olur, karışık maddeleme, çok konulu evrak) ve 2 temiz kontrol içerir (ayrıntı: `data/README.md` §2d).

| Metrik | Adversarial v4 (16 evrak) — TEMİZ ÖLÇÜM |
|---|---|
| Sınıflandırma doğruluğu | 0,938 (15/16); macro-F1 0,933 |
| Birim yönlendirme doğruluğu | 0,938 (15/16) |
| Eksik bilgi tespiti micro-F1 | **1,000** (TP 4, FP 0, FN 0) |
| Mevzuat önerisi isabet@3 | 0,938 (15/16) |
| Taslak kalitesi (hakem) | 94,7 (asgari 82) |
| Evrak başına medyan süre | 0,082 sn |

**Genelleme kanıtı ve dürüst kalan hatalar:** Üç düzeltme de bağımsız evrende genelledi — kopuk İlgi doğru "eksik" işaretlendi, sözel tarih ("Ağustos ayının üçüncü günü") tarihi eksik saymadı, üçüncü-şahıs T.C. kimlikli tutanak ve rapora 6698 önerildi; vatandaşın kendi verisini taşıyan dilekçede usul mevzuatı korunduğundan zarar oluşmadı (eksik-bilgi F1 v3'te 0,667 iken v4'te 1,000). Kalan üç hata düzeltme kapsamı dışındaki, v3 §6'da dokümante sınırlılıklarla birebir örtüşür ve dürüstçe raporlanır: (1) `ust_yazi_b1` → cevap_yazisi (İlgi'deki ikinci-şahıs "yazınız" sinyali; §6-e), (2) `ust_yazi_b2` → strateji (çok konulu makam talimatının yönlendirme belirsizliği), (3) `rapor_b1` mevzuatında 5018 yerine arşiv/RYY (rapor gövde söz dağarcığı; §5.1-c benzeri).
...
otonomi katmanı kurar; 632 birim/entegrasyon testiyle korunur (`pytest tests/`, 16.07.2026).
...
- **Adversarial ölçümün (v3, §5.1) ortaya koyduğu sınırlılıklardan üçü GİDERİLMİŞTİR** (ilkesel düzeltmeler, §5.1.1; dev'de doğrulandı, temiz v4 setinde ölçüldü): (a) İlgi alanı artık METİNSEL varlıkla değil, iki noktalı alan etiketiyle (yapısal blok) denetlenir — kopuk İlgi zinciri tespit edilir; (b) tamamen sözel tarihler ("... ayının ... günü") tarih çıkarıcının kapsamına alınmıştır; (d) belge doğrulanmış T.C. kimlik/IBAN içeriyorsa 6698, "kişisel/kvkk" sözcükleri geçmese bile veri-tespit sinyaliyle önerilir. **Kalan iki sınırlılık** (düzeltilmemiş, v4'te de gözlenmiştir): (c) rapor sonuç/değerlendirme denetimi fiil-kökü listesine dayandığından farklı cümle iskeletlerinde yanlış alarm üretebilir ve rapor gövde söz dağarcığı 5018 gibi usul mevzuatını ilk üçten itebilir; (e) cevap/üst yazı ayrımındaki ikinci-şahıs iyelik ("yazınız") sinyali, İlgi'li talep yazılarında sınır durumlar üretebilmektedir. Bu ikisi ile çok konulu evrakların yönlendirme belirsizliği, düşük-güven kapısı ve insan-onayı mekanizmasıyla yönetilmekte olup gelecek çalışmaya bırakılmıştır.
